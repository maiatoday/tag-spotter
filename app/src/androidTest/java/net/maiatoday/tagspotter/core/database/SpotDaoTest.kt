...
class SpotDaoTest {
    ...
    fun insertAndUpdateSpotArtists() = runBlocking {
        ...
        Assert.assertEquals(updatedArtists, details?.spot?.artists)
    }
    ...
    fun cascadingDeleteOfImagesAndNotes() = runBlocking {
        ...
        Assert.assertEquals(2, details?.images?.size)
        Assert.assertEquals(1, details?.notes?.size)
        ...
        Assert.assertTrue(newDetails?.images?.isEmpty() ?: false)
        Assert.assertTrue(newDetails?.notes?.isEmpty() ?: false)
    }
    ...
    fun databaseQueryFlows() = runBlocking {
        ...
        Assert.assertEquals(3, allSpots.size)
        Assert.assertEquals(3L, allSpots[0].spot.id)
        Assert.assertEquals(2L, allSpots[1].spot.id)
        Assert.assertEquals(1L, allSpots[2].spot.id)

        // Test getAllSpotsDetailsByCategory
        val graffitiSpots = dao.getAllSpotsDetailsByCategory("graffiti").first()
        Assert.assertEquals(2, graffitiSpots.size)
        Assert.assertEquals(3L, graffitiSpots[0].spot.id)
        Assert.assertEquals(1L, graffitiSpots[1].spot.id)
        ...
        Assert.assertTrue(allTags.contains(expected1))
        Assert.assertTrue(allTags.contains(expected2))
    }
    ...
    fun setMainImageTransaction() = runBlocking {
        ...
        Assert.assertTrue(image11?.isMain == true)
        Assert.assertTrue(image10?.isMain == false)
        ...
        Assert.assertTrue(image10New?.isMain == true)
        Assert.assertTrue(image11New?.isMain == false)
    }
}