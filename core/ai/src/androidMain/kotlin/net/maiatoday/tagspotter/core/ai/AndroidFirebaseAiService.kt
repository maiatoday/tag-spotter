...
class AndroidFirebaseAiService(
    ...
    override suspend fun identifyArtist(
        ...
        var byteArray = photoProcessor.decodeScaledBitmap(imagePath, 1024)
        if (byteArray == null && !thumbnailPath.isNullOrEmpty()) {
            byteArray = photoProcessor.decodeScaledBitmap(thumbnailPath, 1024)
        }
        if (byteArray == null) {
            return null
        }
